...
public class GuiBetterAchievements extends GuiScreen {
    ...
    public void initGui() {
        ...
        if (page instanceof ICustomScale) this.scale = ((ICustomScale) page).setScale();
    ...
    protected void keyTyped(char c, int i) {
        if (i == this.mc.gameSettings.keyBindInventory.getKeyCode()) {
            ...
            if (this.currentPage - this.tabsOffset < 0) this.tabsOffset -= maxTabs;
            if (this.tabsOffset < 0) this.tabsOffset = 0;
            ...
            if (this.currentPage - this.tabsOffset >= maxTabs) this.tabsOffset += maxTabs;
            if (this.pages.size() <= this.tabsOffset) this.tabsOffset = this.pages.size() - 1;
        } else {
            super.keyTyped(c, i);
        }
    }
    ...
    protected void actionPerformed(GuiButton button) {
        switch (button.id) {
            ...
            case buttonNext:
                this.tabsOffset += maxTabs;
                if (this.tabsOffset > this.pages.size()) this.tabsOffset = 0;
    ...
    private void handleMouseInput(int mouseX, int mouseY, AchievementPage page) {
        ...
        if (onTab(mouseX, mouseY) != -1) doTabScroll();
        else doZoom(page);
        if (this.xPos < minDisplayColumn) this.xPos = minDisplayColumn;
        if (this.xPos > maxDisplayColumn) this.xPos = maxDisplayColumn;
        if (this.yPos < minDisplayRow) this.yPos = minDisplayRow;
        if (this.yPos > maxDisplayRow) this.yPos = maxDisplayRow;
        if (Mouse.isButtonDown(0)) handleMouseClick(mouseX, mouseY);
    }
    ...
    private void drawAchievementsBackground(AchievementPage page) {
        ...
        if (page instanceof ICustomBackground) {
            GL11.glPushMatrix();
            ((ICustomBackground) page).drawBackground(
                    this.left,
                    this.top,
                    innerWidth + borderWidthX,
                    innerHeight + borderWidthY,
                    this.zLevel,
                    this.scale);
            GL11.glPopMatrix();
        } else {
            ...
            for (int y = 1; y * scale - antiJumpY < innerHeight + borderWidthY; y++) {
                ...
                for (int x = 1; x * scale - antiJumpX < innerWidth + borderWidthX; x++) {
                    ...
                    if (r == 40) {
                        if (random.nextInt(3) == 0) block = Blocks.diamond_ore.getIcon(0, 0);
                        else block = Blocks.redstone_ore.getIcon(0, 0);
                    } else if (r == 20) block = Blocks.iron_ore.getIcon(0, 0);
                    else if (r == 12) block = Blocks.coal_ore.getIcon(0, 0);
                    else if (r > 60) block = Blocks.bedrock.getIcon(0, 0);
                    else if (r > 4) block = Blocks.stone.getIcon(0, 0);
                    else if (r > 0) block = Blocks.dirt.getIcon(0, 0);

                    this.drawTexturedModelRectFromIcon(
                            x * blockSize - antiJumpX,
                            y * blockSize - antiJumpY,
                            block,
                            blockSize,
                            blockSize);
                }
    ...
    private void drawArrow(Achievement achievement, int colourCantUnlock, int colourCanUnlock, int colourUnlocked) {
        ...
        if (depth < 5) {
            ...
            if (unlocked) colour = colourUnlocked;
            else if (canUnlock) colour = colourCanUnlock;
            ...
            this.mc.getTextureManager().bindTexture(Resources.GUI.SPRITES);
            GL11.glEnable(GL11.GL_BLEND);
            if (achievementXPos > parentXPos) this.drawTexturedModalRect(
                    achievementXPos - achievementInnerSize / 2 - arrowHeadHeight,
                    achievementYPos - arrowOffset,
                    arrowRightX,
                    arrowRightY,
                    arrowHeadHeight,
                    arrowHeadWidth);
            else if (achievementXPos < parentXPos) this.drawTexturedModalRect(
                    achievementXPos + achievementInnerSize / 2,
                    achievementYPos - arrowOffset,
                    arrowLeftX,
                    arrowLeftY,
                    arrowHeadHeight,
                    arrowHeadWidth);
            else if (achievementYPos > parentYPos) this.drawTexturedModalRect(
                    achievementXPos - arrowOffset,
                    achievementYPos - achievementInnerSize / 2 - arrowHeadHeight,
                    arrowDownX,
                    arrowDownY,
                    arrowHeadWidth,
                    arrowHeadHeight);
                    ...
                    achievementXPos - arrowOffset,
                    achievementYPos + achievementInnerSize / 2,
                    arrowUpX,
                    arrowUpY,
                    arrowHeadWidth,
                    arrowHeadHeight);
    ...
    private void drawAchievements(AchievementPage page, int mouseX, int mouseY) {
        ...
        for (Achievement achievement : achievements) {
            ...
            if (onAchievement(achievement, mouseX, mouseY)) this.hoveredAchievement = achievement;
        }
        GL11.glPopMatrix();
    }
    ...
    private void drawAchievement(Achievement achievement) {
        ...
        if (unlocked) brightness = 0.75F;
        else if (canUnlock) brightness = 1.0F;
        else if (depth < 3) brightness = 0.3F;
        else if (depth < 4) brightness = 0.2F;
        else if (depth < 5) brightness = 0.1F;
        else return;
        ...
        } else GL11.glColor4f(brightness, brightness, brightness, 1.0F);
        this.mc.getTextureManager().bindTexture(Resources.GUI.SPRITES);
        GL11.glEnable(GL11.GL_BLEND);
        if (special) this.drawTexturedModalRect(
                achievementXPos - achievementOffset,
                achievementYPos - achievementOffset,
                achievementX + achievementTextureSize,
                achievementY,
                achievementTextureSize,
                achievementTextureSize);
        else this.drawTexturedModalRect(
                achievementXPos - achievementOffset,
                achievementYPos - achievementOffset,
                achievementX,
                achievementY,
    ...
    private void drawMouseOverAchievement(int mouseX, int mouseY) {
        ...
        if (iconReset && Mouse.isButtonDown(2)) {
            AchievementRegistry.instance().registerIcon(
                    this.pages.get(this.currentPage).getName(),
                    this.hoveredAchievement.theItemStack,
                    true);
        }
        ...
        if (this.hoveredAchievement instanceof ICustomTooltip)
            ...
            if (!canUnlock) {
                if (depth > 3) return;
                else desc = this.getChatComponentTranslation(
                        "achievement.requires",
                        this.hoveredAchievement.parentAchievement.func_150951_e());

                if (depth == 3) title = I18n.format("achievement.unknown");
            }
            ...
            if (unlocked) this.fontRendererObj.drawStringWithShadow(
                    I18n.format("achievement.taken"),
                    tooltipX,
    ...
    private void doTabScroll() {
        ...
        if (dWheel < 0) this.tabsOffset--;
        else if (dWheel > 0) this.tabsOffset++;
    ...
    private void doZoom(AchievementPage page) {
        ...
        if (dWheel < 0) this.scale += scaleJump;
        else if (dWheel > 0) this.scale -= scaleJump;
    ...
    private void doDrag(int mouseX, int mouseY) {
        if (Mouse.isButtonDown(0)) {
            if (inInnerScreen(mouseX, mouseY)) {
                if (this.newDrag) this.newDrag = false;
                else {
